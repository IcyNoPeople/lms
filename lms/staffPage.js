...
Router.post('/reject/:id', (req, res) => {
    if (req.session.loggedin && req.session.staff) {
        ...
        let updateRejectSQL = 'update loan set approver_id = ?, status = "rejected" where loan_id = ?';
        mydatabase.query(updateRejectSQL, [req.session.sta_id, loan_id], (err, result) => {
            console.log('updated!')
            if (err) {
                return res.status(500).send(err);
            }
